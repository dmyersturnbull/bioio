...
public class BedParser implements LineParser<BedFeature> {
	...
	public BedFeature apply(String line) throws BadDataFormatException {
		...
		try {
			...
			if (parts.length > 4) {
				builder.setScore(new BigDecimal(parts[4]));
			}
			if (parts.length > 5) {
				builder.setStrand(Strand.lookupBySymbol(parts[5]).orElse(null));
			}
			if (parts.length > 6) {
			...
			if (parts.length > 8) {
				builder.setColor(parts[8]);
			}
			if (parts.length > 9) {
				int blockCount = Integer.parseInt(parts[9]);
				String[] starts = sf_comma.split(parts[10]);
				String[] lengths = sf_comma.split(parts[11]);
